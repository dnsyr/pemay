...
$queryPet = "SELECT * FROM (
                SELECT a.*, ROWNUM rnum FROM (
                    ...
                    h.TINGGI, h.LEBAR, h.PEMILIKHEWAN_ID
                    FROM HEWAN h 
                    LEFT JOIN PEMILIKHEWAN p ON h.PEMILIKHEWAN_ID = p.ID 
                    WHERE LOWER(h.NAMA) LIKE LOWER(:search)
                    ORDER BY h.ID
...
<body>
  ...
  <div class="pb-6 px-12 text-[#363636]">
    ...
    <div class="bg-[#FCFCFC] border-base-300 rounded-box p-6">
      ...
      <div class="flex justify-center mt-4 gap-2">
        <?php if($page > 1): ?>
          <a href="?id=<?php echo $id; ?>&page=<?php echo $page-1; ?>&search_pet=<?php echo urlencode($searchPet); ?>" 
        ...
        <?php for($i = 1; $i <= $totalPages; $i++): ?>
          <a href="?id=<?php echo $id; ?>&page=<?php echo $i; ?>&search_pet=<?php echo urlencode($searchPet); ?>" 
  ...
  <div class="drawer drawer-end z-10">
    <input id="drawerUpdatePet" type="checkbox" class="drawer-toggle" />
    <div class="drawer-content">
      <!-- Page content here -->
      <label for="drawerUpdatePet" class="drawer-button btn bg-[#D4F0EA] w-14 h-14 flex justify-center text-[#363636] hover:bg-[#363636] hover:text-[#D4F0EA] items-center rounded-full fixed bottom-5 right-5 border border-[#363636] shadow-md shadow-[#717171]">
        <i class="fas fa-edit fa-lg"></i>
      </label>
    </div>

    <div class="drawer-side">
      <label for="drawerUpdatePet" aria-label="close sidebar" class="drawer-overlay"></label>
      <form method="POST" class="h-full">
        <div class="menu bg-[#FCFCFC] text-[#363636] min-h-screen w-96 flex flex-col justify-center px-8">
          <div class="flex items-center gap-2 mb-7">
            ...
            <h3 class="text-lg font-semibold">Update Pet Data</h3>
          </div>
          ...
          <div class="gap-5 flex flex-col">
            <div class="form-control">
              ...
              <select id="pemilikId" name="pemilikId" class="select select-bordered w-full rounded-full bg-[#FCFCFC] border border-[#565656] text-[#565656] text-sm" required>
                ...
                <?php foreach ($pemilikList as $pemilik): ?>
                  <option value="<?php echo $pemilik['ID']; ?>" <?php echo $data['PEMILIKHEWAN_ID'] == $pemilik['ID'] ? 'selected' : ''; ?>>
            ...
            <div class="form-control">
              ...
              <input type="text" class="mt-1 w-full rounded-full bg-[#FCFCFC] border border-[#565656] text-[#565656] text-sm placeholder:text-[#565656] placeholder:text-sm px-7 py-2" name="spesies" value="<?php echo htmlspecialchars($data['SPESIES']); ?>" required>
            </div>
            ...
            <div class="form-control">
              ...
              <input type="text" class="mt-1 w-full rounded-full bg-[#FCFCFC] border border-[#565656] text-[#565656] text-sm placeholder:text-[#565656] placeholder:text-sm px-7 py-2" name="ras" value="<?php echo htmlspecialchars($data['RAS']); ?>" required>
            </div>
            ...
            <div class="form-control">
              ...
              <input type="number" class="mt-1 w-full rounded-full bg-[#FCFCFC] border border-[#565656] text-[#565656] text-sm placeholder:text-[#565656] placeholder:text-sm px-7 py-2" name="berat" step="0.01" value="<?php echo htmlspecialchars($data['BERAT']); ?>" required>
            </div>
            ...
            <div class="form-control">
              ...
              <input type="date" class="mt-1 w-full rounded-full bg-[#FCFCFC] border border-[#565656] text-[#565656] text-sm placeholder:text-[#565656] placeholder:text-sm px-7 py-2" name="tanggallahir" value="<?php echo date('Y-m-d', strtotime($data['TANGGALLAHIR'])); ?>" required>
            </div>
            ...
            <div class="form-control">
              ...
              <input type="number" class="mt-1 w-full rounded-full bg-[#FCFCFC] border border-[#565656] text-[#565656] text-sm placeholder:text-[#565656] placeholder:text-sm px-7 py-2" name="tinggi" step="0.01" value="<?php echo htmlspecialchars($data['TINGGI']); ?>" required>
            </div>
            ...
            <div class="form-control">
              ...
              <input type="number" class="mt-1 w-full rounded-full bg-[#FCFCFC] border border-[#565656] text-[#565656] text-sm placeholder:text-[#565656] placeholder:text-sm px-7 py-2" name="lebar" step="0.01" value="<?php echo htmlspecialchars($data['LEBAR']); ?>" required>
            </div>
  ...
  <script>
    ...
    window.onload = function() {
      document.getElementById('drawerUpdatePet').checked = true;
    }
  </script>
</body>
</html>
